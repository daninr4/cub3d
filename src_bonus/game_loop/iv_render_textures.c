...
static void	set_x_wall_coordinate(t_game *game, t_mlx_image **texture)
{
	...
	if (player->side == 0)
	{
		*texture = select_texture(game, player, player->rc_dir_x);
		player->x_w_coord = player->pos_y \
			+ player->wall_distance * player->rc_dir_y;
	}
	else
	{
		*texture = select_texture(game, player, player->rc_dir_y);
...
void	render_textures(t_game *game)
{
	...
	while (x < game->mlx_data.width)
	{
		...
		if (x == game->mlx_data.width / 2)
		{
			game->player_data.mid_ray_x = game->player_data.map_x;
			game->player_data.mid_ray_y = game->player_data.map_y;
			game->player_data.mid_wall_distance
				= game->player_data.wall_distance;
		}
