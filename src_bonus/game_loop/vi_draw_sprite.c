...
static void	put_color_sprite(t_mlx_image sprite, t_mlx_data *md, \
								t_sprites_data *sd)
{
	int			c;

	c = mlx_pixel_get(&sprite, sd->tex_x, sd->tex_y);
	if (c != CLR_BACKGROUND)
		mlx_pixel_set(&(md->image), sd->stripe, sd->y, c);
}

static t_mlx_image	select_sprite(t_game *game, int type)
{
	t_textures	*tex;

	tex = &(game->scene_data.textures);
	if (type == BARREL_SPRITE)
		return (tex->barrel[(int) game->mlx_data.time % 3]);
	else
		return (tex->sprite);
}

static void	loop_vertical_stripe(t_game *game, t_sprites_data *sd, int type)
{
	t_mlx_data	*md;
	t_mlx_image	sprite;

	sprite = select_sprite(game, type);
	md = &(game->mlx_data);
	sd->stripe = sd->d_start_x;
	while (sd->stripe < sd->d_end_x)
	{
		sd->tex_x = (int)(256 * (sd->stripe - (-1 * sd->s_width / 2 \
			+ sd->sscreen_x)) * sprite.width / sd->s_width) / 256;
		sd->y = sd->d_start_y;
			...
			&& sd->transform_y < (game->player_data.z_buffer)[sd->stripe])
		{
			while (sd->y < sd->d_end_y)
			{
				...
				sd->tex_y = ((sd->d * sprite.height) / sd->s_height) / 256;
				put_color_sprite(sprite, md, sd);
				(sd->y)++;
			}
		}
		(sd->stripe)++;
...
static void	calculate_sprite(t_game *game, t_sprites_data *data, int type)
{
	data->s_height = \
	...
	loop_vertical_stripe(game, data, type);
}
...
void	draw_sprite(t_game *game, int type)
{
	...
	calculate_sprite(game, data, type);
}
