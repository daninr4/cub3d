...
/*   Updated: 2021/04/12 21:51:03 by jcarrete         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ff_copy_coords(double x, double y, double *new_x, double *new_y)
{
	*new_y = y;
	*new_x = x;
	return (TRUE);
...
void	ff_print_map(t_game *game, int **hash)
{
	...
	y = 0;
	x = 0;
	while (y < game->scene_data.map_data.height)
	{
		x = 0;
		while (x < game->scene_data.map_data.width)
		{
